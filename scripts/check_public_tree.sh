...
if git grep -I -l -i -E '(my|owner|personal|brokerage|portfolio|holding|account).{0,80}(cost[ _-]*basis|account[ _-]*balance|position[ _-]*(value|size)|share[ _-]*quantity|shares|account[ _-]*(id|number)).{0,40}[$]?[0-9]' -- '*.md' '*.json' '*.csv' '*.tsv' '*.yaml' '*.yml' '*.txt' ':!scripts/check_public_tree.sh' ':!scripts/test_public_tree_guard.sh'; then
