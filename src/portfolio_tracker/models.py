...
class TaxFormImport(Base):
    ...
    data. Owner attribution is via `recipient_name` so future [redacted] 1099s
...
class BriefJobStatus(StrEnum):
    """Lifecycle states for a monthly brief generation job."""
...
class MonthlyBriefJob(Base):
    """Tracks an async brief-generation job.

    The Opus call for a monthly brief takes 30-60s, far too long to keep
    a synchronous HTTP request open across page reloads or flaky
    networks. `POST /briefs/generate` instead inserts a row here, spawns
    the actual generation as a FastAPI BackgroundTask, and returns the
    `job_id` so the frontend can poll `GET /briefs/jobs/{job_id}`.

    Lifecycle: pending → running → (succeeded | failed). `brief_id` is
    populated on success; `error` is populated on failure.

    Concurrency guard at the route level: starting a new job for a
    `period_yyyymm` that already has a `pending`/`running` row is
    rejected with 409. We keep the historical rows around (don't delete
    on success) so the dashboard can show "last regenerated by …".
    """

    __tablename__ = "monthly_brief_jobs"
    __table_args__ = (
        Index("ix_monthly_brief_jobs_status", "status"),
        Index("ix_monthly_brief_jobs_period", "period_yyyymm", "started_at"),
    )
